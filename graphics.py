		...
		for event in pygame.event.get():
			if event.type == pygame.KEYDOWN:  
				...
				if(grid[row][column]==0):
					grid[row][column] = 1
					pygame.draw.rect(screen,RED,[(MARGIN + CELL_WIDTH) * column + MARGIN,(MARGIN + CELL_HEIGHT) * row + MARGIN,CELL_WIDTH,CELL_HEIGHT])
				else:
					grid[row][column] = 0
					pygame.draw.rect(screen,WHITE,[(MARGIN + CELL_WIDTH) * column + MARGIN,(MARGIN + CELL_HEIGHT) * row + MARGIN,CELL_WIDTH,CELL_HEIGHT])
				clock.tick(60)
				pygame.display.flip()
...
def loopfunction(grid):
	...
	if len(Xarr) > 0:
		...
		for i in range(len(Xarr)):
			...
			color = GREEN
			pygame.draw.rect(screen, color,[(MARGIN + CELL_WIDTH) * Yarr[i] + MARGIN,(MARGIN + CELL_HEIGHT) * Xarr[i] + MARGIN,CELL_WIDTH,CELL_HEIGHT])
			clock.tick(60)
			pygame.display.flip()
			currX = Xarr[i]
			currY = Yarr[i]
...
# Taking source and destination details as input from the user
