...
// A Function to trace the path from the source to destination 
...
{ 	
	...
	//inputting and outputting stuf from and to files.
	fstream map;
	fstream position;
	ofstream myfile;
	...
	/* Description of the Grid-input 
	1--> The cell is not blocked 
	any other integer--> The cell is blocked */
	/* Top left is (0,0) and
	1)moving right will increase
	j by 1 unit
	2)moving down will increase
	i by 1 unit
	*/
	int grid[ROW][COL];
	int inp_count = 0;
	...
	if(inp_count < height*width){
		//if insufficient input is given OR 
		//if there is some problem with the input file.
